...
var require_sweetalert_min = __commonJS((exports2, module2) => {
  !function(t, e) {
    typeof exports2 == "object" && typeof module2 == "object" ? module2.exports = e() : typeof define == "function" && define.amd ? define([], e) : typeof exports2 == "object" ? exports2.swal = e() : t.swal = e();
  }(exports2, function() {
    return function(t) {
      function e(o) {
        if (n[o])
...
// game.ts
var import_sweetalert = __toModule(require_sweetalert_min());
(function() {
  var CSS = {
    arena: {
      width: 900,
  ...
  function drawScoreBoard() {
    $("<span/>", {id: "left"}).text(`left: ${CONSTS.score1} `).appendTo("#score-board");
    $("<span/>", {id: "right"}).text(`right: ${CONSTS.score2} `).appendTo("#score-board");
  }
  ...
  function loop() {
    interval = window.pongLoop = setInterval(function() {
      ...
      if (CSS.ball.left <= CSS.stick.width) {
        if (CSS.ball.top + CSS.ball.height < CSS.stick1.top || CSS.ball.top > CSS.stick1.top + CSS.stick.height) {
          updateScore(++CONSTS.score2, "right");
          saveGame();
          roll();
        }
        if (CSS.ball.top + CSS.ball.height / 2 > CSS.stick1.top && CSS.ball.top + CSS.ball.height / 2 < CSS.stick1.top + CSS.stick.height) {
          CONSTS.ballLeftSpeed = CONSTS.ballLeftSpeed * -1;
        }
      }
      if (CSS.ball.left >= CSS.arena.width - CSS.ball.width - CSS.stick.width) {
        if (CSS.ball.top + CSS.ball.height < CSS.stick2.top || CSS.ball.top > CSS.stick2.top + CSS.stick.height) {
          updateScore(++CONSTS.score1, "left");
          saveGame();
          roll();
        }
        if (CSS.ball.top + CSS.ball.height / 2 > CSS.stick2.top && CSS.ball.top + CSS.ball.height / 2 < CSS.stick2.top + CSS.stick.height) {
          CONSTS.ballLeftSpeed = CONSTS.ballLeftSpeed * -1;
        }
      }
      checkGameEnd();
  ...
  function checkGameEnd() {
    ...
    if (winner) {
      (0, import_sweetalert.default)(`${winner} wins.

Score: Left: ${CONSTS.score1}, Right: ${CONSTS.score2}

Play again?`).then((playAgain) => {
        if (playAgain) {
          newGame();
  ...
  function playWithCpu(id, cpuCount) {
    ...
    observer2.observe(document.getElementById(id), {
      attributes: true
    });
    return observer2;
  }
  function selectGameType(loopCb) {
    return new Promise((resolve, reject) => {
      try {
        (0, import_sweetalert.default)("Select Game Type", "", {
          buttons: {
            youVsYou: {
              text: "You(L) vs You(R)",
              value: "YOU_VS_YOU"
          ...
          if (value == "LOAD_SAVE_GAME") {
            let saveGame2 = loadSaveGame();
            if (saveGame2) {
              (0, import_sweetalert.default)("Save game found", "Game started", "success");
              CONSTS = saveGame2;
              clearScoreBoard();
              loopCb();
              resolve(CONSTS.gameTypeSelected);
            } else {
              (0, import_sweetalert.default)("Save game not found", "Refresh the page", "warning");
              resolve(value);
            }
          } else {
